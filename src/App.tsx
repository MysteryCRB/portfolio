...
import {
  ...
  X,
  Phone,
  ExternalLink // Added to fix the rendering issue
} from 'lucide-react';
...
const rotatingIcons = [
  { id: 'github', icon: <Github size={80} />, label: 'GitHub' },
  { id: 'email', icon: <Mail size={80} />, label: 'Email' },
  { id: 'linkedin', icon: <Linkedin size={80} />, label: 'LinkedIn' },
  { id: 'whatsapp', icon: <Phone size={80} />, label: 'WhatsApp' }
];
...
const App: React.FC = () => {
  ...
  // For rotating 3D icons on desktop
  ...
  useEffect(() => {
    ...
    if (typingElement) {
      ...
      typeCommand();
      const commandInterval = setInterval(typeCommand, 10000);
  ...
  // 3D icon rotation effect every 5 seconds (desktop only)
  useEffect(() => {
    const iconInterval = setInterval(() => {
      // Trigger a short spin animation, then switch the icon
      if (iconRef.current) {
        iconRef.current.classList.remove('spin');
        void iconRef.current.offsetWidth; // reflow to restart animation
  ...
  return (
    <div className="relative min-h-screen bg-white text-[#212121] overflow-hidden">
      {/* Desktop Navigation */}
      <nav className="hidden md:flex md:fixed top-6 left-1/2 transform -translate-x-1/2 z-50 bg-white bg-opacity-80 backdrop-blur-md px-8 py-4 rounded-full shadow-lg">
        <ul className="flex flex-row space-x-6">
          <li>
            <button
              onClick={() => scrollToSection('hero')}
              className={`nav-link ${activeSection === 'hero' ? 'font-bold' : ''}`}
            >
              Home
            </button>
          </li>
          <li>
            <button
              ...
              className={`nav-link ${activeSection === 'about' ? 'font-bold' : ''}`}
            >
              About
            </button>
          </li>
          <li>
            <button
              ...
              className={`nav-link ${activeSection === 'experience' ? 'font-bold' : ''}`}
            >
              Experience
            </button>
          </li>
          <li>
            <button
              onClick={() => scrollToSection('organizational')}
              className={`nav-link ${activeSection === 'organizational' ? 'font-bold' : ''}`}
            >
              Organizations
            </button>
          </li>
          <li>
            <button
              ...
              className={`nav-link ${activeSection === 'projects' ? 'font-bold' : ''}`}
            >
              Projects
            </button>
          </li>
          <li>
            <button
              ...
              className={`nav-link ${activeSection === 'contact' ? 'font-bold' : ''}`}
            >
              Contact
            </button>
          </li>
      ...
      <nav className="fixed top-6 right-6 md:hidden z-50">
        <button
          onClick={() => setIsNavOpen(true)}
          className="p-2 bg-white bg-opacity-80 backdrop-blur-md rounded-full shadow-lg"
        >
          <Menu size={24} />
        </button>
      </nav>
      ...
      {isNavOpen && (
        <div className="fixed inset-0 bg-white bg-opacity-95 z-50 flex flex-col items-center justify-center animate-slideDown">
          <button onClick={() => setIsNavOpen(false)} className="absolute top-6 right-6 p-2">
            <X size={28} />
          </button>
          <ul className="space-y-6 text-2xl text-center">
            ...
            <li>
              <button onClick={() => scrollToSection('organizational')} className="nav-link">
                Organizations
              </button>
            </li>
            <li>
      ...
      <section id="hero" className="min-h-screen flex items-center justify-center relative">
        <div className="text-center space-y-6 animate-fade-in">
          ...
          <h1 className="text-5xl font-bold hover:text-[#78909C] transition-colors duration-300">
            ALMENDO GABRIEL TETELEPTA
          </h1>
      ...
      <section id="about" className="py-16">
        <div className="container mx-auto px-4">
          <h2 className="text-4xl font-bold mb-12 reveal-on-scroll border-b-2 border-[#78909C] pb-2">
            About Me
          </h2>
            ...
            <div className="space-y-6">
              ...
              <div className="company-card hover-effect">
                ...
                <ul className="mt-6 space-y-3 text-[#424242]">
                  ...
                  <li className="flex items-center gap-2">
                    <span>•</span> Top 80 out of 1200+ students in campus-wide English test
                  </li>
                  <li className="flex items-center gap-2">
                    <span>•</span> Active participant in 10+ university events
                  </li>
                </ul>
              </div>
              ...
              <div className="experience-card relative hover-effect">
                ...
                <ul className="list-none space-y-3">
                  <li className="flex items-start gap-3">
                    ...
                    <p>
                      Configured and managed Kaspersky Security Center and deployed Kaspersky Antivirus
                      Endpoints companywide
                    </p>
                  </li>
                  <li className="flex items-start gap-3">
                    <div className="min-w-4 mt-1">
      ...
      <section id="organizational" className="py-16">
        <div className="container mx-auto px-4">
          <h2 className="text-4xl font-bold mb-12 reveal-on-scroll border-b-2 border-[#ECEFF1] pb-2">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            <div className="org-card reveal-on-scroll hover-effect">
              <div className="p-6">
                ...
                <p className="mb-2">President University • Jul 2023 - Dec 2023</p>
            ...
            <div className="org-card reveal-on-scroll hover-effect">
              <div className="p-6">
                ...
                <p className="mb-2">Cikarang, Indonesia • Sep 2023 - Nov 2023</p>
                <ul className="list-none space-y-2">
                  <li className="flex items-start gap-3">
                    <div className="min-w-4 mt-1">
                      <div className="h-2 w-2 bg-[#78909C] rounded-full"></div>
      ...
      <section id="contact" className="py-16 relative">
        <div className="container mx-auto px-4">
          ...
          {/* 3D Spinning Icons on the right (desktop only) */}
          <div className="hidden md:block absolute top-1/2 right-20 transform -translate-y-1/2">
            <div
              ref={iconRef}
              className="icon-3d text-[#424242] w-[100px] h-[100px] flex items-center justify-center hover3D"
            >
              {rotatingIcons[currentIconIndex].icon}
            </div>
          </div>
        </div>
      </section>
      ...
      <style jsx>{`
        ...
        /* Hover Effect for Icons and Cards */
        .hover-effect {
        ...
        /* 3D Spinning Icon */
        .icon-3d {
          perspective: 800px; /* so we can see the 3D rotation */
        }
        .spin {
          animation: spin3D 1.5s ease;
        }
        @keyframes spin3D {
          0% {
        ...
        /* Hover up/down effect on the 3D icon container */
        .hover3D:hover {
          animation: upDownHover 1s ease-in-out infinite;
        }
      `}</style>
